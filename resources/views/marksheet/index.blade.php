                    ...
                    <div class="row">
                        ...
                        <div class="col-md-3 col-xs-6 col-sm-3">
                            <div class="form-group">
                                ...
                                <select id="selectBatch" class="js-example-basic-single form-control @error('batch_id') is-invalid @enderror" name="@can('marksheet.trainee_result') batch_id_trainee @else batch_id @endcan" >
                        ...
                        <!-- <div class="row">
                            <div class="col-md-12 col-xs-12 col-sm-12">
                                <div id="view" class="form-group clearfix">
...
<script>
    ...
    $(document).ready(function() {
        $('select[name=" batch_id_trainee "]').on('change', function() {
            var batch_id = $(this).val();
            
            if (batch_id) {
                $.ajax({
                    url: '/marksheet/trainee/' + batch_id ,
                    type: "GET",
                    dataType: "json",
                    success: function(data) {
                        var res = '<option value="">Select Trainee</option>';
                        $.each(data.trainees, function(key, value) {
                            res +=
                                '<option value="'+ value.id +'">'+ value.name +'</option>';
                        });
                        $('select[name="trainee_id"]').html(res);
                    }
    ...
    $(document).ready(function() {
        $('select[name=" batch_id "]').on('change', function() {
            var batch_id = $(this).val();
            $.get('/marksheet/examresult/' + batch_id , function(data) {
            ...
            // if (batch_id) {
            //     $.ajax({
            //         url: '/marksheet/examresult/' + batch_id ,
            //         type: "GET",
            //         dataType: "json",
            //         success: function(data) {
            //             $('tbody[name="body"]').html(res);
            //         }
            //     });
    ...
    $(document).ready(function() {
        $('select[name="trainee_id"]').on('change', function() {
            var trainee_id = $(this).val();
            $.get('/marksheet/traineeresult/' + trainee_id , function(data) {
                $data = $(data); // the HTML content that controller has produced
                $('#view').hide().html($data).fadeIn();
            });
            event.preventDefault();
